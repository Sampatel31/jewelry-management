...
export const createCreditNote = async (req: Request, res: Response) => {
  try {
    ...
    if (invoice.finalization_status !== FINALIZED) {
      return res.status(400).json({ message: 'Credit notes can only be issued against a finalized invoice' });
    }
    ...
    await auditLog({ userId, action: 'CREATE', tableName: 'credit_notes', recordId: id, newValues: note, ipAddress: req.ip });
...
export const getCreditNotes = async (req: Request, res: Response) => {
  try {
    const notes = await db('credit_notes')
      .where({ invoice_id: req.params.id })
      .join('users', 'credit_notes.issued_by', 'users.id')
      .select('credit_notes.*', 'users.name as issued_by_name')
